...
### v1.0.13（最新）
- **子页面边缘返回修复（双重保障）**：新增通用边缘返回手势组件 EdgeBackGesture，包裹全部一级/二级子页面（设置、书签、历史、下载、缓存、自定义拦截、离线页面、阅读、标签切换页），在原生 Cupertino 交互式返回手势受 WKWebView 平台视图干扰失效时，仍可从屏幕左边缘右滑返回；带防重复触发（原生手势进行中不二次 pop）。各子页面左上角保留系统返回按钮作为点击兜底
- **深色模式网页白屏修复**：弃用不可控的 overrideUserInterfaceStyle（只对声明适配深色的网站生效，导致"导航栏黑、网页白"割裂），改为注入强制深色 CSS（反色方案，图片/视频/画布等媒体二次反色还原），网页内容随深色模式整体变暗；每次加载完成自动应用、切换即时生效；原生 WKWebView/scrollView 底色同步设为深色，消除加载间隙与弹性区域白闪
- **翻译黑屏修复**：移除 overrideUserInterfaceStyle 后，翻译只替换文本节点、与深色 CSS 相互独立，不再触发整页变黑
- **修复本地翻译词库**：修复 en_zh_dict_basic.json 因网页端追加导致的 JSON 结构损坏（两段对象非法拼接，会使本地词库整体加载失败），合并为合法扁平词典（20997 条），补回基础词条 world
- 子页面 AppBar 硬编码浅色背景统一改为跟随深色主题

### v1.0.12
- **边缘手势修复**：更多菜单进入的全部一级/二级页面（书签、历史、离线页面、设置、下载管理、缓存管理、自定义拦截等）路由统一改为 iOS 原生 CupertinoPageRoute，恢复屏幕边缘右滑返回手势
- **重力感应默认开启**：移除设置页重力感应开关，更多菜单默认支持长按拖拽排序（保留拖拽手柄提示与 iOS 触感反馈），菜单顺序仍持久化保存
- **深色模式修复**：① 切换主题即时全局刷新（地址栏、底部工具栏、标签切换页、设置分组、下载/缓存/拦截等全部页面同步）；② 新建与休眠恢复的 WKWebView 自动同步深色渲染，修复标签重建后网页主题错乱；③ 新标签页、底部弹窗、菜单列表完整适配深浅色配色
